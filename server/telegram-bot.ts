...
🚀 Ready to start? Click the button below to open the game!
      ...
      const keyboard = {
        reply_markup: {
          inline_keyboard: [
            [{ text: '🎮 Play KushKlicker', web_app: { url: `${process.env.REPLIT_DOMAINS || 'http://localhost:5000'}?ref=${username}` } }],
...
Keep clicking to earn more! 🚀
        ...
        const keyboard = {
          reply_markup: {
            inline_keyboard: [
              [{ text: '🎮 Continue Playing', web_app: { url: process.env.REPLIT_DOMAINS || 'http://localhost:5000' } }]
            ]
          }
        };
    ...
    bot.onText(/\/leaderboard/, async (msg) => {
      ...
      try {
        ...
        const keyboard = {
          reply_markup: {
            inline_keyboard: [
              [{ text: '🎮 Play Now', web_app: { url: process.env.REPLIT_DOMAINS || 'http://localhost:5000' } }]
            ]
          }
        };
